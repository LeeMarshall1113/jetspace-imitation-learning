...
def main() -> int:
    ...
    for ep in range(args.episodes):
        ...
        # The repository is mid-migration: main carries PR #9's
        # buffer_size(), while this branch still has __len__. Supporting both
        # keeps this script runnable on either without a merge first.
        size = buf.buffer_size() if hasattr(buf, "buffer_size") else len(buf)
        if size == 0:
            skipped += 1
            continue
